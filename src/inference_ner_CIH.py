...
def formatting_func_for_inference(row, rag=False):
    """Generate prompt for inference - updated for new format."""
    example = row.get("similar_examples", 
                     "Text: LLLT at 50 mW was more efficient in the modulation of matrix MMPs and tissue repair.\nEntities: <start> LLLT at 50 mW <end>") \
              if (rag and "similar_examples" in row) else \
              "Text: LLLT at 50 mW was more efficient in the modulation of matrix MMPs and tissue repair.\nEntities: <start> LLLT at 50 mW <end>"

    text = (
        "You are an expert NER system for CIH (Complementary and Integrative Health) therapy extraction.\n"
        "Extract ALL Complementary and Integrative Health entities from the input sentence without rephasing.\n"
        "RULES:\n"
        "- Output format:<start> therapy_name; therapy_name <end>.\n"
         "- Use semicolon (;) to separate multiple entities\n"
        "- If no therapies are found, output EXACTLY: <start> None <end>\n\n"
         "DO NOT OUTPUT ANY THINKING, JUST DIRECTLY OUTPUT THE ENTITIES.\n"
        f"Example: {example}\n\n"
        f"### Text: {row['text']}\n"
        f"### Entities:"
    )
...
def extract_entities_from_prediction(text, prompt):
    ...
    UPDATED: Conservative extraction - treats content as single entity by default.
    """
    if prompt in text:
    ...
    if not matches:
        # Fallback: Try to find content after "Entities:" even without tags
        fallback_pattern = r'Entities?:\s*(.+?)(?:\n|$)'
        fallback_matches = re.findall(fallback_pattern, text, re.IGNORECASE)
        if fallback_matches:
            content = fallback_matches[0].strip()
        else:
            return entities
    else:
        content = matches[0].strip()
    
    # Check for empty/none cases
    if content.lower() in ['none', 'no entities', 'no therapies', '']:
        return entities
    
    # CONSERVATIVE APPROACH: Treat entire content as ONE entity
    # Only split if there's strong evidence of multiple distinct therapies
    
    # Check for explicit separators
    if '\n' in content or ';' in content or '|' in content:
        separators = r'[\n;|]'
        parts = re.split(separators, content)
        for part in parts:
            if part.strip() and part.strip().lower() != 'none':
                entities.append({'text': part.strip(), 'type': None})
    
    elif ' and ' in content.lower():
        # Split by "and" if it appears to separate distinct therapies
        parts = re.split(r'\s+and\s+', content, flags=re.IGNORECASE)
        if len(parts) <= 3:  # Reasonable number of entities
            for part in parts:
                if part.strip() and part.strip().lower() != 'none':
                    entities.append({'text': part.strip(), 'type': None})
        else:
            # Keep as one entity
            entities.append({'text': content, 'type': None})
    
    else:
        # DEFAULT: Keep as single entity
        entities.append({'text': content, 'type': None})
...
def clean_entities(ents):
    """Keep only entities with text, type matching no longer required."""
    cleaned = []
    for e in ents:
        text = " ".join(e.get("text", "").split())
        if text:
            # Keep the type if it exists (for gold), otherwise None (for predictions)
            cleaned.append({
                "text": text, 
                "type": e.get("type")
            })
...
def match_entity(gold, pred, mode="exact", fuzzy_thresh=0.8):
    """
    Match entities based on text only (ignore type).
    
    Args:
        gold: gold entity dict with 'text' and optional 'type'
        pred: predicted entity dict with 'text' (type may be None)
        mode: matching mode ('exact', 'overlap', or 'fuzzy')
    """
    gtext, ptext = gold["text"], pred["text"]
...
def evaluate_mode(all_true, all_pred, mode="exact", fuzzy_thresh=0.8):
    """
    Evaluate predictions against ground truth (text-based matching only).
    """
    total_tp = 0
    total_fp = 0
    total_fn = 0
    
    # Track per-type metrics for gold entities only
    ...
    for gold_list, pred_list in zip(all_true, all_pred):
        ...
        for i, p in enumerate(pred):
            for j, g in enumerate(gold):
                if j in matched_g or i in matched_p:
                    continue
                if match_entity(g, p, mode=mode, fuzzy_thresh=fuzzy_thresh):
                    matched_g.add(j)
                    matched_p.add(i)
                    total_tp += 1
                    # Track by gold type if available
                    if g.get("type"):
        ...
        # Count false negatives by gold type
        for j, g in enumerate(gold):
    ...
    # Per-class metrics (based on gold types)
    per_class = {}
    for t in ALLOWED_TYPES:
        ...
        per_class[t] = {
            "precision": prec, "recall": rec, "f1": f1_class,
...
def print_summary(res):
    ...
    print(f"Totals: TP={res['totals']['tp']}, FP={res['totals']['fp']}, FN={res['totals']['fn']}")
    print("\nPer-class breakdown (based on gold types):")
    for t, m in res["per_class"].items():
        print(f"  {t:25s} P={m['precision']:.3f} R={m['recall']:.3f} F1={m['f1']:.3f} "
              f"(support={m['support']}, TP={m['tp']}, FN={m['fn']})")
